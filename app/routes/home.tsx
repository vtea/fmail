...
type HomeActionData = {
	addresses: string[];
	error?: "invalid_address" | "issue_failed";
};
...
export async function action({ request, context }: Route.ActionArgs) {
	...
	if (intent === "restore") {
		...
		if (!isGeneratedEmailAddress(address)) {
			return data({
				addresses,
	...
	if (intent === "generate") {
		const issued = await issueNewAddress(d1);
		if (!issued) {
			return data({
				addresses,
			...
			<fetcher.Form method="post" className="space-y-2">
				...
				<div className="flex flex-col gap-2 sm:flex-row sm:items-center">
					...
					<input
						...
						className="border-theme-soft bg-theme-subtle text-theme-primary w-full rounded-xl border px-3 py-2.5 text-sm outline-none sm:flex-1"
					/>
					<button
						type="submit"
...
export default function Home({ loaderData, actionData }: Route.ComponentProps) {
	...
	const actionError = ((): HomeActionData["error"] => {
		...
		if (value === "invalid_address" || value === "issue_failed") {
			return value;
		}
		return undefined;
	})();
