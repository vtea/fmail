...
- `app/routes/`：路由模块（首页、Markdown 页、Sitemap、API 等）。
...
## 真实数据架构（必须遵循）
- D1：邮件元数据（`emails`）与已发放地址（`issued_addresses`）。
- R2：存邮件原始内容（对象 key = 邮件 `id`）。
- Cookie Session：存当前临时邮箱地址、签发时间等。
...
- 遵循现有代码风格：
  - 缩进使用 Tab。
  - 保持现有文件风格一致，避免无关格式化。
- 路由命名遵循当前约定（如 `sitemap.xml.tsx`、`robots.txt.tsx`）。
