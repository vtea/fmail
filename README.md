...
3. 首页按当前 Cookie Session 中的地址读取 D1 列表；生成新地址会写入 `issued_addresses`，避免重发已用地址
...
```text
app/
  routes/              # 路由模块（home、md、api、sitemap、robots 等）
...
- `pnpm run deploy`：构建 + 远端 D1 迁移 + 部署到 Cloudflare Workers
...
- `D1`：邮件元数据 `emails` 与已发放地址 `issued_addresses`（数据库名 `smail-v3`）
